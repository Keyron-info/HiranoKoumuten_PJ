...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 2. Force Create/Update Users
        users_to_fix = [
            {'email': 'tanaka@hira-ko.jp', 'role': 'department_manager', 'last': '田中', 'first': '一朗'},
            {'email': 'maki@hira-ko.jp', 'role': 'managing_director', 'last': '眞木', 'first': '宣之'},
            {'email': 'sakai@hira-ko.jp', 'role': 'president', 'last': '堺', 'first': '仁一郎'},
            {'email': 'honjo@oita-kakiemon.jp', 'role': 'senior_managing_director', 'last': '本城', 'first': '美代子'},
            {'email': 'takeda@hira-ko.jp', 'role': 'accountant', 'last': '竹田', 'first': '鉄也'},
        ]
        ...
        steps = [
            ...
            {'order': 5, 'name': '専務承認', 'pos': 'senior_managing_director', 'user': created_users.get('senior_managing_director')},
            {'order': 6, 'name': '経理承認', 'pos': 'accountant', 'user': None},
        ]
